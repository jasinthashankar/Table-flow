...
const NewReservation = () => {
  ...
  return (
    <div className="new-reservation-page max-w-4xl mx-auto px-4 py-8 space-y-6 page-enter">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="reservation-panel space-y-5">
            <h2 className="reservation-panel__title">
                  ...
                  <button
                    ...
                    className={`p-3 text-left border rounded-lg transition-all cursor-pointer ${
                      seatingPreference === pref.type
                        ? 'reservation-choice is-selected'
                        : 'reservation-choice'
                    }`}
                  >
                    <span className="reservation-choice__label capitalize">{pref.label}</span>
                    <span className="reservation-choice__desc">{pref.desc}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="reservation-panel space-y-4">
            <h2 className="reservation-panel__title">
            ...
            {isLoading ? (
                    ...
                    <button
                      ...
                      className={`p-2.5 border rounded-lg text-xs font-semibold text-center transition-all cursor-pointer ${
                        selectedSlot === slot.timeSlot
                          ...
                          : isAvailable
                          ? 'reservation-slot'
                          : 'reservation-slot is-disabled'
                      }`}
                    >
                      <div>{slot.timeSlot}</div>
                      <div className="reservation-slot__meta">
              ...
              <p className="reservation-empty">No operational slots available for this selection.</p>
            )}
          </div>
        </div>

        {/* Right Column: Contact Details & Submit */}
        <div className="reservation-panel space-y-4 h-fit">
          <h2 className="reservation-panel__title">
